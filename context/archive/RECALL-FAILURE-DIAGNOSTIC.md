...
> **➕ 2026-05-31 — 3 MORE recall-defect cases surfaced by Ragas (Run B), not by recall score.** The full sweep above used the custom Gemini judge's recall score as the trigger (≤3). Run B (Ragas, same frozen fixture) flagged **3 additional cases the custom judge had scored recall=4/5 (a pass)** — Ragas's claim-decomposed `LLMContextRecall` scored them low (MOB-003=0.33, STR-003=0.50, STR-007=0.50). Chunk-level verification confirmed all 3 are **genuine retrieval defects the holistic custom judge was too generous on**: **MOB-003 = reference-pollution** (3/5 top-5 are Behm bibliography chunks), **STR-003 = wrong-chunk selection** (the fact-dense Latella Results chunk idx 8 never enters top-20), **STR-007 = single-paper saturation** (non-expected Montoro-Bombú takes 10/20, crowding out expected Dudagoitia entirely). Each also has a test-authoring component. **Full per-fact chunk targets in `RETRIEVAL-TARGET-CHUNKS.md` Cases 13-15.** Lesson: a second eval implementation surfaced retrieval problems the primary judge's score masked — these expand the Phase-2 candidate surface and the test-cleanup list.

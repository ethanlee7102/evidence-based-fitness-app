...
5. ✅ **DONE (2026-05-31) — Ragas integration.** Ragas 0.4.3 in a **dedicated `venv-ragas`** (it needs `langchain-community<0.4`, conflicts with the app's langchain 1.x — isolated via `requirements-ragas.txt`; the frozen fixture JSON is the handoff boundary so the eval tool never imports the app pipeline). New: `scripts/capture_rag_fixture.py` (freezes RAG outputs once → `results/rag_outputs_fixture.json`, 100/100, 0 fail), `--from-fixture` on the custom judge, `src/core/ragas_runner.py` + `scripts/evaluate_rag_ragas.py`. Embeddings = `gemini-embedding-001` (text-embedding-004 404s on this key). Metric map: `nv_context_relevance/context_recall/llm_context_precision_with_reference/answer_relevancy/faithfulness` → our 5 keys. Native 0–1 scores stored.
6. ✅ **DONE (2026-05-31) — Run B (Ragas + Gemini)** on the frozen fixture → `results/run_b_ragas_gemini.json` (98 scored, 0 fail, overall 0.86). Custom judge re-run on the *same* fixture → `results/run0_custom_fixture.json` (4.56, on top of the 4.58 live baseline — fixture is faithful). Both judges independently rank **recall weakest, precision 2nd**.
7. ✅ **DONE (2026-05-31) — Analysis writeup.** `scripts/analyze_eval_agreement.py` → `results/eval_agreement_analysis.md`. **Findings:** Axis-1 (model) Claude lenient on retrieval (recall Δ+0.12 norm ≈ +0.48 pts), lockstep on answer-quality. Axis-2 (implementation) **recall correlates strongly (r=0.73)** — the Phase-2-driving metric is implementation-robust; answer-quality metrics show `n/a (saturated)` correlation (ceiling-clustered, zero variance) but high within-1pt agreement (89–94%) → use agreement-rate, not correlation, where saturated. Custom judge validated as not self-confirming. **Caveat still holds: keep Gemini fixed as the Phase-2 retrieval judge.**

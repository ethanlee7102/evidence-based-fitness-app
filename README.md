...
- Questions that compare two topics can be answered incompletely. A single search for a question like "A versus B" is dominated by whichever topic is more common in the corpus, so the less common topic can get crowded out of the results even when the library does contain relevant papers on it. The system then answers the well-covered side without flagging that the other side is thin, because it does not yet check whether the retrieved passages actually cover the whole question. The planned fix is to split comparative questions into per-topic sub-queries, so each topic gets its own search, and to add a sufficiency check that scopes the answer to what the sources support.

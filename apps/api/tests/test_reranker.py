...
def _chunk(chunk_id: str, paper_id: str, text: str = "text", similarity: float = 0.5) -> ChunkResponse:
...
def test_retrieve_reranked_orchestration(monkeypatch):
    ...
    out = _run(rv.retrieve_reranked("q", top_n=5, fetch_depth=150, ef_search=500, per_paper_cap=2))
    ...
    # Cap=2 breaks P's monopoly: keep p0,p1 then q0,q1 then r0 — p2,p3 chopped.
